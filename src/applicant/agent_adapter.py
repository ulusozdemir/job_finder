    ...
    async def apply(self, url: str, profile: ApplicantProfile) -> ApplyResult:
        try:
            ...
            task_prompt = (
                ...
                f"SECURITY CHECKS & CAPTCHA HANDLING:\n"
                f"- If LinkedIn asks for an email verification code, call "
                f"'get_linkedin_verification_code' to retrieve it, then enter it.\n"
                f"- Cloudflare / CAPTCHA: If you see a Cloudflare 'Verify you are human' checkbox, "
                f"'Just a moment...', or any CAPTCHA challenge:\n"
                f"  1. Click the checkbox ONCE and wait 5 seconds.\n"
                f"  2. If the page loads → continue.\n"
                f"  3. If still stuck (shows 'Verifying...' then resets) → try ONE more click + wait.\n"
                f"  4. If still stuck after 2 attempts → IMMEDIATELY report "
                f"done(text='CAPTCHA_BLOCKED: Cloudflare challenge could not be solved', success=False).\n"
                f"  Do NOT retry more than 2 times. Automated browsers cannot solve Cloudflare Turnstile.\n"
                f"- For any other CAPTCHA (image puzzle, distorted text): report CAPTCHA_BLOCKED immediately.\n"
                f"- Do NOT keep retrying the same action if it triggers a login popup — report failure.\n"
                ...
                # ── FIELD ORDERING & PRE-FILLED VALUES ──
                f"FIELD ORDERING & PRE-FILLED VALUES (CRITICAL — READ CAREFULLY):\n"
                f"Many forms have DEPENDENT FIELDS (e.g. City depends on Country, District depends "
                f"on City). A child field will show NO options until its parent is actively selected.\n"
                f"RULES:\n"
                f"1. ALWAYS fill fields TOP-TO-BOTTOM as they appear on screen.\n"
                f"2. A dropdown is NOT 'filled' unless YOU selected a value from its suggestions "
                f"in THIS session. If a dropdown already shows a value when the page loaded "
                f"(e.g. Country shows 'Türkiye'), that value is NOT registered by the framework. "
                f"You MUST click it, type the value, and select from suggestions BEFORE moving "
                f"to any child field.\n"
                f"3. If a child field shows no suggestions after typing, the FIRST thing to check "
                f"is whether you actively selected its parent dropdown. If not, go back and select "
                f"the parent first, then retry the child.\n"
                f"4. NEVER skip a dropdown that has a pre-filled value. Always re-select it.\n\n"
                ...
                f"- 'input' action + force_click_element: For dropdown/autocomplete/typeahead fields. "
                f"Type with 'input' → see suggestions → use force_click_element(text='option text') "
                f"to click the option. This uses trusted browser events that React Select needs. "
                f"Do NOT use regular 'click' for dropdown options — it uses synthetic events that "
                f"frameworks like React Select ignore. Do NOT use fill_text_field either.\n"
