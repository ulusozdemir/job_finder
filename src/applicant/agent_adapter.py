    ...
    async def apply(self, url: str, profile: ApplicantProfile) -> ApplyResult:
        try:
            ...
            task_prompt = (
                ...
                f"- Do NOT use find_elements to discover indices — it does NOT return interactive indices. "
                ...
                f"IMPORTANT: Before retrying a field, check if it ALREADY has the correct value "
                f"in the screenshot. If a radio looks selected but a validation error persists, "
                f"the error may be about a DIFFERENT field. Read all error messages carefully.\n\n"
